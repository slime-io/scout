...
import scalapb.json4s.base64.Base64Variant.MIME_NO_LINEFEEDS
...
class Printer private (config: Printer.PrinterConfig) {
  ...
  def serializeSingleValue(
    ...
    case PByteString(v) =>
      JString(MIME_NO_LINEFEEDS.encode(v.toByteArray))
...
object JsonFormat {
  ...
  def parsePrimitive(
    ...
    case (Type.TYPE_BYTES, JString(s)) =>
      PByteString(
        ByteString.copyFrom(MIME_NO_LINEFEEDS.decode(s))
      )
    case _ => onError
  }
  ...
  private[json4s] object MemorizedFieldNameMap {
    ...
    : mutable.Map[Descriptor, Map[String, FieldDescriptor]] =
      mutable.Map.empty
